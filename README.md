...
| **Integration Layer** | Channel adapters, 6 MCP servers | WebChat + optional Telegram, Slack, WhatsApp via `bindings` |
...
The system implements the **Four-Layer LLMOS Paradigm** with a **Multi-Agent Model** (v0.3.0) that separates cognitive workload:
...
openclaw-akos/
  akos/                             Shared orchestration library (Python)
    __init__.py                     Package marker + version (v0.3.0)
...
# Full suite (190+ tests) -- pyproject.toml configures discovery
py -m pytest -v
