...
- **HLK duplicate `item_name` cleanup** — Added **`scripts/dedupe_ambiguous_process_item_names.py`** (operator-run) to disambiguate legacy display-name collisions so **`ambiguous_item_names`** is empty and **`scripts/backfill_process_parent_ids.py`** reports zero parent-id holes; USER_GUIDE §24.3.4 documents **`PROCESS_LIST_FIELDNAMES`** for external CSV tooling.
