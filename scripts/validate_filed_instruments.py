#!/usr/bin/env python3
"""Validate FILED_INSTRUMENTS.csv (Initiative 21 / P5 + I81 P2 T3).

Cross-checks discipline FK, GOI counterparty FK, owner_role FK, supersedes
chain, status enum, and date format.

Renamed from ``validate_founder_filed_instruments.py`` at I81 P2 T3
(D-IH-81-S under D-IH-81-G umbrella, 2026-05-23). The old script path
``scripts/validate_founder_filed_instruments.py`` is retained as a
deprecation shim for one initiative cycle (removal at I81 P9 closure).

Usage: py scripts/validate_filed_instruments.py
"""
...
_INSTRUMENTS_NEW = HLK_COMPLIANCE / "advops" / "FILED_INSTRUMENTS.csv"
_INSTRUMENTS_LEGACY = HLK_COMPLIANCE / "FOUNDER_FILED_INSTRUMENTS.csv"
INSTRUMENTS_CSV = _INSTRUMENTS_NEW if _INSTRUMENTS_NEW.is_file() else _INSTRUMENTS_LEGACY
...
def main() -> int:
    print("\n  FILED_INSTRUMENTS Validator")
    print("  " + "=" * 40)
    if not INSTRUMENTS_CSV.is_file():
        print("  FAIL: FILED_INSTRUMENTS.csv not found at advops/ or legacy path")
        return 1
    ...
    with open(INSTRUMENTS_CSV, encoding="utf-8", newline="") as f:
        ...
        if reader.fieldnames != list(FILED_INSTRUMENTS_FIELDNAMES):
            print("  FAIL: header mismatch")
            print(f"    expected: {list(FILED_INSTRUMENTS_FIELDNAMES)}")
