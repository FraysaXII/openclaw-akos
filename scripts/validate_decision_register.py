...
- decision_id matches the standard ^D-IH-\\d{1,3}-[A-Z](-V\\d+)?$ pattern OR the
  closure-decision pattern ^D-IH-\\d{1,3}-Decision-P\\d+(-[A-Z0-9-]+)?-\\d{4}-\\d{2}-\\d{2}$.
...
OPS_CSV = REPO_ROOT / "docs" / "references" / "hlk" / "compliance" / "OPS_REGISTER.csv"
