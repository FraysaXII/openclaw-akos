...
- **Initiative closure:** If a Cursor plan or `master-roadmap.md` promised **browser / WebChat / Langfuse UI / Docker** UAT, record dated **`reports/uat-*.md`** outcome tables (PASS / SKIP / N/A) or explicit skips—**not** only “run the DEVELOPER_CHECKLIST gates.” Full contract: [`.cursor/rules/akos-planning-traceability.mdc`](../../.cursor/rules/akos-planning-traceability.mdc) (if that file is not yet patched, see [planning/99-proposals/cursor-rules-uat-evidence-instructions.md](planning/99-proposals/cursor-rules-uat-evidence-instructions.md)).
