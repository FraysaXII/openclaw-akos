...
| `OPS-73-1` | INIT-OPENCLAW_AKOS-73 — I73 — People Operations + Engagement Models + Methodology IP (mega-initiative) | mixed (People Operations Lead) | — | Strand E ENGAGEMENT_MODEL_REGISTRY canonical CSV mint + ENGAGEMENT_REGISTRY 17-col extension + process_list tbi_peopl_dtp_engagement_* tranche + Supabase mirror DDL + Pydantic SSOT + validator + tests + sync-script extension | Closure target P1 (~3 days). PAUSE POINT #1 - canonical-CSV gate per akos-governance-remediation.mdc HLK compliance governance. D-IH-73-H..N mint at P1 per-class enum + 17-col extension ratifications. |
...
| `OPS-73-2` | INIT-OPENCLAW_AKOS-73 — I73 — People Operations + Engagement Models + Methodology IP (mega-initiative) | mixed (Learning Curator) | — | Strand A Learning charter + Holistik Researcher onboarding curriculum + LEARNING_OPS_BACKLOG.csv cohort tracking | Closure target P2 (~2 days). Inline-ratify gates: C-73-1 cohort size (default 1); C-73-2 versioning anchor (default methodology-anchor per I71 P4). |
