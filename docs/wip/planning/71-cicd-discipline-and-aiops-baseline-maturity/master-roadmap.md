...
### P4 — Strand C2 (review-stamp dimension) **SHIPPED 2026-05-14**

- **Scope (SHIPPED)**: applied **C-71-4 column-extension verdict per default** (mirrored tables get column-extension; unmirrored canonicals get separate-table). 4 mirrored canonical CSVs gained the 4 review-stamp columns (`last_review_at` / `last_review_by` / `last_review_decision_id` / `methodology_version_at_review`): `process_list.csv` (Process subject class) + `DECISION_REGISTER.csv` (Decision subject class) + `INITIATIVE_REGISTRY.csv` + `OPS_REGISTER.csv` (Registry-row subject class governance trio). `CANONICAL_REGISTRY.csv` (Artifact subject class) **deferred** to follow-up commit since no `compliance.canonical_registry_mirror` exists today (standalone-table path applies).
- **Prerequisites (met)**: P3 closed (`392e050`); `D-IH-71-E` (P0); `validate_compliance_schema_drift.py` PASS state.
- **Deliverables (SHIPPED 2026-05-14)**: design ratification doc at `reports/p4-design-2026-05-14.md` (10 sections: authority + C-71-4 verdict + 4-column shape + DDL + RLS + PII + CSV+akos.* updates + validator + ERP panel slot + test plan + risk + cross-references); SQL audit trail at `reports/sql-proposal-p4-review-stamp-2026-05-14.md` (Operator SQL Gate per `akos-holistika-operations.mdc` discover-propose-execute pattern); migration `supabase/migrations/20260514193709_i71_p4_review_stamp.sql` applied via Supabase MCP `apply_migration` (16 ALTER TABLE ADD COLUMN statements; idempotent `IF NOT EXISTS` guards; one transaction); 4 CSV headers extended + 4 `akos/hlk_*_csv.py` SSOT tuples extended in lockstep per `akos-docs-config-sync.mdc` "Any canonical compliance CSV header change" contract; `scripts/validate_review_stamps.py` (~470 LOC) freshness-window validator (180-day stale + 30-day grace for missing-stamp + invalid-decision-ref error rule); release-gate.py INFO row (advisory only; never blocks); NEW sidecar `docs/wip/planning/REVIEW_STAMP_INBOX.md` auto-rendered with idempotent dated section between `<!-- BEGIN/END REVIEW-STAMP-AUTO -->` markers; `HLK_ERP_ARCHITECTURE.md` §4 reserves `/operator/governance/freshness-dashboard/` panel slot (implementation deferred to I72+); 29-case test suite at `tests/test_validate_review_stamps.py` (PASS); `D-IH-71-Q` minted (134 active decisions); `OPS-71-3` closed (`closure_decision_id: D-IH-71-Q`; `closed_at: 2026-05-14`); `INIT-OPENCLAW_AKOS-71` notes appended (P4 SHIPPED); CHANGELOG `[Unreleased] / Added` entry; phase report at `reports/p4-strand-c2-review-stamp-2026-05-14.md`.
- **Verification (run 2026-05-14)**: `validate_compliance_schema_drift.py` PASS (22 canonicals tuple-aligned including the 4 P4-extended); `validate_review_stamps.py` PASS (1212 info / 0 warning / 0 error advisories across 1364 rows); `validate_hlk.py` PASS; `validate_decision_register.py` PASS (134 active decisions); `validate_initiative_registry.py` PASS; `validate_ops_register.py` PASS (`OPS-71-3` closed cleanly); `validate_canonical_registry.py` PASS; brand pytest marker suite PASS; `release-gate.py` Verdict per phase report §"Verification matrix"; Supabase advisors (security + performance) returned 0 P4-attributable issues.
- **Inline-ratify gate (C-71-4) — RATIFIED at execution per default**: column-extension where mirror exists; separate-table for unmirrored canonicals. Operator's blanket trust signal at P4 kickoff pre-approved the default; no AskQuestion surfaced (lower-friction option). Verdict: column-extension applied to 4 mirrored canonicals; standalone-table path deferred to follow-up commit for the Artifact subject class (`CANONICAL_REGISTRY` not mirrored).

### P4 follow-up (Round 2) — review-stamp expansion + Artifact standalone-table **SHIPPED 2026-05-14**

- **Scope (SHIPPED)**: rolling-chore Round-2 follow-up to P4 commit `bb04f08`. Two combined deliverables ratified in one decision row (`D-IH-71-R`): (A) extend the 4-column review-stamp shape to the **17 remaining mirrored compliance.\* canonicals** beyond the 4 already extended at P4; (B) build the **standalone-table path** for the Artifact subject class via `compliance.review_stamps_standalone` (the path deferred at P4 per the C-71-4 column-extension-where-mirror-exists default; first-stamp trigger fires at this commit).
- **Prerequisites (met)**: P4 closed (`bb04f08`); `validate_compliance_schema_drift.py` PASS post-P4; `D-IH-71-Q` ratification on record.
- **Deliverables (SHIPPED 2026-05-14)**: design ratification doc at `reports/p4-followup-design-2026-05-14.md` (mirror enumeration table + per-mirror column-extension or N/A decision + Artifact standalone-table architecture + validator extension plan + test plan); SQL audit trail at `reports/sql-proposal-p4-followup-2026-05-14.md` (Operator SQL Gate per `akos-holistika-operations.mdc` discover-propose-execute pattern; 17 mirrors enumerated + 7 mirrors excluded with rationale + CANONICAL_REGISTRY confirmed unmirrored); migration `supabase/migrations/20260514202912_i71_p4_followup_review_stamp_expansion.sql` applied via Supabase MCP `apply_migration` (68 ALTER TABLE ADD COLUMN statements + 1 CREATE TABLE for `compliance.review_stamps_standalone` with 10 columns + RLS service_role-only ALL policy + INSERT backfill 66 rows from CANONICAL_REGISTRY.csv `last_review`; one transaction; idempotent `IF NOT EXISTS` guards + `ON CONFLICT DO NOTHING`); 17 CSV headers extended + 17 `akos/hlk_*_csv.py` SSOT tuples extended in lockstep per `akos-docs-config-sync.mdc`; `scripts/validate_review_stamps.py` registry expanded from 4 to 21 entries + new `_scan_canonical_md_artifacts()` helper for the Artifact scan path (reads CANONICAL_REGISTRY.csv `last_review`); 28-case test extension at `tests/test_validate_review_stamps.py` (`TestRegistryExpansion` + `TestColumnExtensionMigrationExpanded` × 17 + `TestArtifactScan` × 5 + `TestComplianceSchemaDriftRegression` + `TestRealCanonicalsSmokeExpanded` × 3); `D-IH-71-R` minted (135 active decisions); `OPS-71-3` stays CLOSED at P4 (no state change at follow-up); `INIT-OPENCLAW_AKOS-71` notes appended (P4-followup SHIPPED); CHANGELOG `[Unreleased] / Added` entry; phase report at `reports/p4-followup-review-stamp-expansion-2026-05-14.md`.
- **Verification (run 2026-05-14)**: `validate_compliance_schema_drift.py` PASS (22 canonicals tuple-aligned including the 17 newly-extended); `validate_review_stamps.py` PASS (1716 info / 0 warning / 0 error advisories across 2025 rows; Artifact scan: 92 rows stamped); `pytest tests/test_validate_review_stamps.py -v` PASS (57 cases); `validate_decision_register.py` PASS (135 active decisions); `validate_initiative_registry.py` PASS; `validate_canonical_registry.py` PASS; Supabase MCP `get_advisors` returned 0 P4-followup-attributable issues.
- **Mirrors not extended at follow-up (rationale)**: 4 taxonomy lookups (`access_level` / `confidence_level` / `source_category` / `source_level` — not CSV mirrors); 3 operational facts (`validation_runs` / `eval_run` / `dossier_run` — explicit NOT git SSOT comments; per-row stamping breaks append-only contract); CYCLE_REGISTER.csv (akos.* tuple exists but no Supabase mirror; standalone-table pattern is generic enough to absorb it later via `subject_kind='cycle_csv'` without further DDL).
...
4. **C-71-4 — Strand C2 column vs table**: **RATIFIED 2026-05-14 at P4 execution per default** (operator blanket-trust pre-approved default at P4 kickoff; no AskQuestion surfaced — lower-friction option). **Verdict: column-extension on 4 mirrored canonicals (process_list / decision_register / initiative_registry / ops_register) per `D-IH-71-Q`; standalone-table path deferred to follow-up commit when first unmirrored canonical needs a stamp (Artifact subject class via `CANONICAL_REGISTRY` is the first deferral target since no `compliance.canonical_registry_mirror` exists today).**
...
- **D-IH-71-Q** — **MINTED 2026-05-14** Strand C2 column-vs-table ratification (P4); column-extension verdict per C-71-4 default; 4 mirrored canonicals extended; freshness-window validator live; ERP panel slot reserved; OPS-71-3 closed. 134 active decisions.
...
- [x] Strand C2 (SHIPPED 2026-05-14): review-stamp migration applied (`supabase/migrations/20260514193709_i71_p4_review_stamp.sql`; 16 ALTER TABLE ADD COLUMN across 4 mirror tables); freshness-window validator green (`scripts/validate_review_stamps.py`; INFO row in release-gate; 1212 info / 0 warning / 0 error advisories on first run); ERP panel slot `/operator/governance/freshness-dashboard/` reserved in `HLK_ERP_ARCHITECTURE.md` §4. `D-IH-71-Q` minted; `OPS-71-3` closed.
