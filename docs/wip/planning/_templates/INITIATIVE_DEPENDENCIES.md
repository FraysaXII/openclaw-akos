...
| **I81** — Knowledge-base integrity sweep (vault + planning surface) + Compliance layout reorganisation + named-milestone migration + full-vault SOP retrofit | candidate | Operator charter + **baseline/process approval** gates for CSV path migrations; named-milestone schema (`D-IH-81-H`) ratified at P0; I80 P6 forward-charter MET | **I80 P7 closed** (`D-IH-80-CLOSURE`) **+ KNOWLEDGE_PAIRING pattern live** (`D-IH-80-H`) | **I82** (consumes integrity matrix + unlocks Confidence in folder layout hygiene) **+ all future initiatives** (named-milestone schema is permanent vocabulary, codified in `akos-planning-traceability.mdc` §"Plan-quality bar") **+ downstream mirror/ERP path consumers** | — | candidate (scope expanded twice operator 2026-05-16: morning Wave 1 = vault integrity + Compliance layout; evening Wave 2 = planning-surface integrity + named-milestone migration + Class B validator) |
