...
Initiative folders under `docs/wip/planning/` use a **two-digit prefix** (`01-` … `05-`, plus `99-proposals/`) so sort order and “what came before” are obvious in the filesystem. The table below matches that layout. **Ad-hoc** `.plan.md` files live in `99-proposals/`.
...
| **04** | [`holistika-company-formation/`](04-holistika-company-formation/) | Founder / incorporation / governance planning. |
...
- New **program** initiative: add the next **06**, **07**, … row here and create `planning/NN-<kebab-slug>/` (next free `NN`) with a `master-roadmap.md` per [`docs/wip/README.md`](../README.md).
