...
3. **process_list tranche ratified**: 7 `tbi_peopl_dtp_engagement_*` rows owned by `People Operations Lead` with paired-SOP forward-charter via `TODO[I73-P2-SOP-PATH]` or `TODO[I73-P3-SOP-PATH]` markers per D-IH-72-W feature-flag pattern. Cadence: 3× scheduled (quarterly), 4× event_triggered. No baseline_organisation tranche required (all 3 forward-charter roles — People Operations Lead, Learning Curator, Ethics Advisor — already exist from I70 P8.5 D-IH-70-Q split).
