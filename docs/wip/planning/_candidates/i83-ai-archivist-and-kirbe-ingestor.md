...
> **Update 2026-05-17 — I84 P4 pre-ratification cascade (D-IH-84-E closed; C-83-1 narrowed-to-2 finalists).** I84 P4 ratified **E1 — LlamaIndex-continue + LangGraph-workflow** as the KiRBe-ingestor framework narrowing per [`decision-log.md` D-IH-84-E](../84-substrate-doctrine-and-commercial-readiness/decision-log.md). **C-83-1 closes to a 2-finalist surface**: I83 P0 picks between LlamaIndex (`SUBS-RUN-LLAMA-LLAMAINDEX`; persistent + retrieval-augmented-generation anchor) and LangGraph (`SUBS-LANGCHAIN-AI-LANGGRAPH`; orchestration-engine + workflow overlay) as the primary orchestration anchor — the likely outcome is **composition of both** (LlamaIndex-for-retrieval + LangGraph-for-workflow-orchestration) since they natively compose via langchain-native tool protocol. Both are MIT open-source aligned with the I84 P4 D-IH-84-B B5 self-governance + retractability + no-vendor-lock-in principle ratified at the same gate. Rationale grounded in (a) past-poc-translation-matrix.md §5.3 explicit recommendation; (b) substrate continuity per SUBSTRATE_LANDSCAPE_DOCTRINE.md §4 principle 2; (c) F3 dispatcher pattern matches the F5 per-task framing ratified at D-IH-84-C for AICs. Cursor SDK + LlamaIndex (the operator's "somehow D" preference in the initial gate) was structurally precluded by the B5 self-governance principle (Cursor SDK is proprietary-SaaS). C-83-2 / C-83-3 / C-83-4 / C-83-5 remain operator-gated at I83 P0.
