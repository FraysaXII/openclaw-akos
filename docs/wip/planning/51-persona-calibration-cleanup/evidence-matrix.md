...
| E6a | I51/P5: `--hard-fail-on-drift` lives in `scripts/calibrate_scenarios.py` (I47/P10 already shipped the flag); I51/P5 wires a new `calibration-drift-gate` job into `.github/workflows/eval-tier-b.yml` (post-`tier-b`, single-invocation, 90-day artifact retention). Pre-commit warn-only invariant preserved — no `.pre-commit-config.yaml` exists in repo. | `.github/workflows/eval-tier-b.yml` post-P5; local `--hard-fail-on-drift --quiet` exit 0 | D-IH-51-C executed |
