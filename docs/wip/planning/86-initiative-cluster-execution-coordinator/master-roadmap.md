...
sibling_coordinated_initiatives:
  ...
  - I83 AI Archivist + KiRBe Ingestor (active)
  - I74 Brand-tooling productization (active)
  - I75 Research-area governance (active)
...
| I83 | INIT-OPENCLAW_AKOS-83 | **active** (Wave O 2026-05-21 per D-IH-86-CC OVERRIDE) | P0 charter | 5 | 6-phase KiRBe ingestor scaffold; blocker-tracker superseded by active promotion — residual gates in initiative master-roadmap. Tracker archived context: docs/wip/planning/_blockers/i83-promotion-blocker-tracker.md. |
| I74 | INIT-OPENCLAW_AKOS-74 | **active** (Wave O 2026-05-21 per D-IH-86-CC OVERRIDE) | P0 charter | 3-4 | Brand-tooling productization; TRIGGER-2 override accepted at promotion. Tracker archived context: docs/wip/planning/_blockers/i74-promotion-blocker-tracker.md. |
| I75 | INIT-OPENCLAW_AKOS-75 | **active** (Wave O 2026-05-21 per D-IH-86-CC OVERRIDE; Radar mint home Wave R+5 C1) | P0 charter | 2 end | Research-area governance + Research Radar 16th QF specialty (D-IH-86-FG). Tracker archived context: docs/wip/planning/_blockers/i75-promotion-blocker-tracker.md. |
...
| Cluster siblings on blocker-tracker (historical) | **0 of 10** | I74 + I75 + I83 promoted active Wave O (D-IH-86-CC); tracker files retained for lineage only |
...
### 1.9 Wave K — 4-layer output architecture (chartered + executed 2026-05-20 per operator regression on Wave J)

Wave K emerged when operator ran a comprehensive **regression on the post-Wave-J doctrine surface** (verbatim 2026-05-20: *"please do a regression over my decisions and or executions to detect flaws or things i haven't taken into account"*) and surfaced **11 gaps across 4 severity tiers**. The agent's regression sweep identified: 3 critical (UAT class taxonomy 7→11; fabric materialisation 5→9 specialties; J-AIC dangling FK), 4 high (compose() runbook gating; UX role ownership clarity; architectural depth gap on "components"; channel registry expansion), 2 medium (PRECEDENCE.md sync; render-script classification), 2 low (changelog enrichment; touchpoint-kit frontmatter upgrade). Operator ratified all 11 fixes simultaneously per scope option C-with-regression-rewire (D-IH-86-BD); operator's clarifying refinement on the "component" framing — *"please bear in mind that we are speaking of components in AI and there could be more scenarios, slides of pdf/pptx, images, voice for agents, reading for different reader platforms, scenarios, etc, excalidraw, mermaids, graphs, gantts, please try to think of a way to properly organize the output type"* — produced the load-bearing architectural pivot: **NOT a 6th axis on the fabric** (the fabric stays at 5 axes), but a **4-layer hierarchy** sitting *beneath* the fabric that decomposes the *output* dimension into orthogonal layers.

**The 4-layer architecture (D-IH-86-BB):**

| Layer | Name | Purpose | Canonical CSV | Library narrative |
|:-----:|:-----|:--------|:--------------|:------------------|
| **1** | Output Type | The *medium / shape* of the output | [`OUTPUT_TYPE_REGISTRY.csv`](../../../references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/dimensions/OUTPUT_TYPE_REGISTRY.csv) (17 codes) | [`OUTPUT_TYPE_LIBRARY.md`](../../../references/hlk/v3.0/Admin/O5-1/People/canonicals/OUTPUT_TYPE_LIBRARY.md) |
| **2** | Artifact Class | The *named purpose* of the output | [`ARTIFACT_CLASS_REGISTRY.csv`](../../../references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/dimensions/ARTIFACT_CLASS_REGISTRY.csv) (21 codes) | [`ARTIFACT_CLASS_LIBRARY.md`](../../../references/hlk/v3.0/Admin/O5-1/People/canonicals/ARTIFACT_CLASS_LIBRARY.md) |
| **3** | Component Primitive | The *Shadcn-shape granular primitives* | [`COMPONENT_PRIMITIVE_REGISTRY.csv`](../../../references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/dimensions/COMPONENT_PRIMITIVE_REGISTRY.csv) (25 codes) | [`COMPONENT_PRIMITIVE_LIBRARY.md`](../../../references/hlk/v3.0/Admin/O5-1/People/canonicals/COMPONENT_PRIMITIVE_LIBRARY.md) |
| **4** | Render Surface | The *delivery surface* (PDF / Web / ERP / Mail / Slide / Broadcast) | (existing 6-surface canonical) | [`akos-external-render-discipline.mdc`](../../../../.cursor/rules/akos-external-render-discipline.mdc) |

**Wave K composition** (12 atomic phases delivered same-day):

- **K1** — Mechanical fixes (J-AIC mint in `AUDIENCE_REGISTRY.csv` resolves Wave I/J dangling-FK; channel count in `HOLISTIKA_QUALITY_FABRIC.md` §4.2 corrected to 10; D-IH-86-AX rationale clarified per UX-Design absorption into Brand & Narrative Manager 2026-05-15).
- **K2** — 6 decisions (D-IH-86-AY through D-IH-86-BD) appended to `DECISION_REGISTER.csv` via [`scripts/_one_off/append_wave_k_decisions.py`](../../../../scripts/_one_off/append_wave_k_decisions.py).
- **K3** — Forward-charter [`I-NN-OUTPUT-ARCHITECTURE`](../_candidates/i-nn-output-architecture.md) candidate file (supersedes the previously-named `I-NN-MESSAGE-COMPONENT-LIBRARY` single-axis framing per D-IH-86-BC).
- **K4** — `HOLISTIKA_QUALITY_FABRIC.md` edits: §3.1 documents 4-layer hierarchy beneath the 5-axis fabric; §6 extends materialisation table 5→9 specialties (adds MKTOPS / TECHOPS / DATAOPS / Output-architecture per D-IH-86-AZ); §10 adds `compose()` runbook gating as charter→active gate item 4 per D-IH-86-BA.
- **K5** — `UAT_DISCIPLINE.md` §4 + §4.1 promote 7→11 UAT classes (adds `localisation` / `accessibility` / `performance` / `privacy` per D-IH-86-AY).
- **K6, K7, K8** — Three new canonical CSVs minted (Layer 1 + Layer 2 + Layer 3) + three paired narrative libraries (`OUTPUT_TYPE_LIBRARY.md` + `ARTIFACT_CLASS_LIBRARY.md` + `COMPONENT_PRIMITIVE_LIBRARY.md`) each carrying skeleton + 1 worked exemplar (OT-PROSE-EMAIL-RICH for L1; AC-COVER-EMAIL with full audience-matrix variant menu for L2; CP-CTA Shadcn-shape with 8 variants × 6 audience-class composition × 8 anti-patterns + open-code templates for mail / DM / web React for L3).
- **K9** — 15 [`touchpoint-kit/`](../../../references/hlk/v3.0/_assets/touchpoint-kit/) intro-message files upgraded with `output_type_source` + `output_type_render` + canonical `artifact_class: AC-INTRO-MESSAGE` + `component_primitive_inventory:` + `layered_architecture_version: D-IH-86-BB` frontmatter via [`scripts/_one_off/backfill_touchpoint_kit_layered_frontmatter.py`](../../../../scripts/_one_off/backfill_touchpoint_kit_layered_frontmatter.py).
- **K10** — All 11 `render_*.py` + 2 `export_*.py` scripts retro-classified into `ARTIFACT_CLASS_REGISTRY.render_script_path` column; `AC-PMO-HUB-RENDER` row extended to cover `render_topic_graph.py` + `render_operational_cohesion_index.py`.
- **K11** — `PRECEDENCE.md` adds 6 canonical entries (3 CSVs + 3 libraries); `CHANGELOG.md` Wave K entry; this §1.9 narrative; `files-modified.csv` backfill.
- **K12** — `validate_hlk.py` + dangling-FK sweep + atomic Wave K commit.

**Wave K doctrine moves crystallised.** (1) The 4-layer architecture is the durable answer to operator's "where do components live; where does UI start; where do other things cross-area start" question — Layer 3 is where every cross-area primitive lives (prose primitives owned by Brand & Narrative Manager; UI primitives owned by Brand & Narrative Manager post-2026-05-15 absorption; mermaid primitives owned by System Owner per plan-quality-bar tradition). (2) Per-component research depth is now contractualised at the library narrative level — every primitive's doctrine page has 9 sections (anatomy / variants / composition / research / a11y / brand / open-code-templates / worked-exemplars / anti-patterns) borrowed from Shadcn + HeroUI principles + adapted for multi-modal output (not just web UI). (3) The audience-channel-scenario-brand-governance fabric *parametrises* the 4 layers — fabric.compose() output names which output_type, which artifact_class, which component_primitives, which render_surface compose any artifact's quality bar. (4) Forward-charter `I-NN-OUTPUT-ARCHITECTURE` activation gates: fabric at active + UAT_DISCIPLINE at active + compose() runbook landing + I-NN-CHANNEL-DOCTRINES P0 (per the narrative-thread continuity doctrine the operator named: *"this is a continuous process of discovery research design determine test mint repeat"*).

### 1.10 Wave L — Output-architecture mechanical hardening + UAT backfill + Supabase mirroring (chartered + executed 2026-05-21 per operator `go all out P0..P5` ratification on Wave K post-flow regression)

Wave L emerged when operator surfaced — verbatim 2026-05-20 — *"are we properly mirroring in Supabase and pushing to git? has the UAT been performed?"* — exposing the doctrine debt that Wave K's same-day landing left open: registries had canonical CSVs + narrative libraries (Layer 1+2+3 architectural skeleton) but no Pydantic SSOT, no composite validator, no Supabase mirror, no UAT closure reports, and no People-DoD pattern propagation. The operator's compounding clarification — *"is full P0 to P5 an option? I prefer to go all out"* + `opt-uat-backfill-now` — ratified the maximalist closure scope: mechanical hardening + UAT backfill + pattern propagation in one atomic commit, while preserving forward-charters for HLK-ERP surfacing (P4) and the ~52 per-row doctrine pages (P5).

**Wave L composition** (six P-phases delivered same-session as one atomic commit):

- **P0** — Side-effects committed (`a9fa9a4`) + Wave J + Wave K + side-effects pushed to `origin/main` (5 commits ahead → in-sync). Resolves operator's "are we pushing to git?" with explicit yes-trail.
- **P1** — Pydantic SSOT chassis: 3 new models at [`akos/hlk_output_type_registry_csv.py`](../../../../akos/hlk_output_type_registry_csv.py) + [`akos/hlk_artifact_class_registry_csv.py`](../../../../akos/hlk_artifact_class_registry_csv.py) + [`akos/hlk_component_primitive_registry_csv.py`](../../../../akos/hlk_component_primitive_registry_csv.py); 20 valid/invalid test pairs at [`tests/test_hlk_output_architecture_registries.py`](../../../../tests/test_hlk_output_architecture_registries.py). `kind` field on component primitives intentionally typed as `str` (not `Literal`) to support semicolon-delimited multi-kind values per `D-IH-86-BF` — token validation against `VALID_KINDS = frozenset({'prose', 'visual', 'interactive', 'mixed'})` happens at validator layer per `D-IH-86-BG`.
- **P2** — Composite validator: [`scripts/validate_output_architecture_registries.py`](../../../../scripts/validate_output_architecture_registries.py) performs (a) header drift; (b) per-row Pydantic; (c) cross-FK against `AUDIENCE_REGISTRY` + `DECISION_REGISTER` + sibling registries. Wired into [`scripts/validate_hlk.py`](../../../../scripts/validate_hlk.py) + [`config/verification-profiles.json`](../../../../config/verification-profiles.json) `pre_commit` + [`scripts/release-gate.py`](../../../../scripts/release-gate.py) PASS/FAIL. **Validator caught + corrected 3 data errors during mint**: AC-DOSSIER comma-escape inside `fabric.compose()` (commas → slashes); AC-RUNBOOK-SCRIPT `output_type_codes` simplified from prose to `OT-PROSE-MARKDOWN`; CP `kind` schema relaxed `Literal` → `str` with token-validation. **Final**: 17 OT + 21 AC + 25 CP rows PASS.
- **P3** — Supabase mirroring: DDL migration [`supabase/migrations/20260521003459_i86_wave_l_output_architecture_mirrors.sql`](../../../../supabase/migrations/20260521003459_i86_wave_l_output_architecture_mirrors.sql) creates 3 mirror tables + 3 governance views + RLS deny-all-except-`service_role`; applied via `plugin-supabase-supabase apply_migration` MCP. [`scripts/sync_compliance_mirrors_from_csv.py`](../../../../scripts/sync_compliance_mirrors_from_csv.py) extended with 3 emit functions + CLI flags + dispatch map. Mirror DML synced via `execute_sql` MCP: **17 OT + 21 AC + 25 CP rows** in MasterData (`swrmqpelgoblaquequzb`); SELECT count verified parity with canonical CSVs.
- **P4** — UAT backfill: 2 closure reports authored using [`_templates/uat-closure-template.md`](../../_templates/uat-closure-template.md). [`reports/uat-wave-j-2026-05-19.md`](reports/uat-wave-j-2026-05-19.md) covers Wave J Quality Fabric meta-doctrine + Vercel hotfix (deploy `dpl_8N4pqRVEhhUCMMV82A8RUzYAfixo` READY); 5 of 7 UAT classes exercised (closure + brand + render + regression + deploy). [`reports/uat-wave-k-2026-05-20.md`](reports/uat-wave-k-2026-05-20.md) covers Wave K 4-layer architecture; 5 of 11 classes exercised including new `accessibility` + `privacy` classes (proves 7→11 promotion was operationalised, not nominal). Both reports `verdict: PASS-WITH-FOLLOWUP` + `closure_decision_source: operator_explicit`.
- **P5** — People-DoD pattern propagation: minted `pattern_4layer_output_architecture_below_quality_fabric` row in [`PEOPLE_DESIGN_PATTERN_REGISTRY.csv`](../../../references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/dimensions/PEOPLE_DESIGN_PATTERN_REGISTRY.csv) (consumer_areas covers 9 areas) per [`akos-people-discipline-of-disciplines.mdc`](../../../../.cursor/rules/akos-people-discipline-of-disciplines.mdc) RULE 1; paired cross-area-breakthrough announcement at [`reports/cross-area-breakthrough-output-architecture-2026-05-21.md`](reports/cross-area-breakthrough-output-architecture-2026-05-21.md) per `SOP-PEOPLE_CROSS_AREA_BREAKTHROUGH_001` contract. Names per-area consumption contracts (Marketing primary; Research / Tech Lab / Operations heavy; Legal / Compliance / Ethics / Finance light) + activation-gate clearance trail.

**Wave L decisions ratified.** Six new rows in [`DECISION_REGISTER.csv`](../../../references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/DECISION_REGISTER.csv): D-IH-86-BE (pattern propagation) + BF (Pydantic SSOT chassis) + BG (composite validator) + BH (Supabase migration) + BI (mirror DML) + BJ (UAT backfill cadence). All decisions reference operator `go all out P0..P5` ratification (2026-05-20).

**Wave L forward-charters preserved.** P6 (HLK-ERP surfacing of the 4-layer architecture into operator inbox + planning workspace panels) and P7 (~52 per-row doctrine pages reaching Shadcn-shape depth) remain forward-chartered inside `_candidates/i-nn-output-architecture.md`; promotion to active master-roadmap remains gated on A1 (compose() runbook) + A2 (UAT 11-class promotion exercised in production) + A3 (first channel doctrine landed). The candidate file's §2.3 *"registries land NOW; doctrine pages mature post-activation"* posture is preserved — Wave L closes the **mechanical** doctrine debt without forcing premature activation of the **content** doctrine work.

**Wave L doctrine moves crystallised.** (1) The Wave K 4-layer architecture is now SSOT-drift-protected at every layer of the responsibility stack — canonical CSV → Pydantic chassis → composite validator → release-gate wiring → Supabase mirror → governance view; future agents cannot append a malformed row without CI failing. (2) UAT backfill discipline (operator's `opt-uat-backfill-now`) becomes precedent for closing future Waves where same-day execution outpaces UAT authoring — the `_templates/uat-closure-template.md` is now battle-tested across 2 Waves with PASS-WITH-FOLLOWUP verdicts. (3) People-DoD propagation runs end-to-end (pattern row + cross-area-breakthrough announcement) — the runbook `peopl_cross_area_breakthrough_announce.py` will fire at next operator session per SOP-PEOPLE_CROSS_AREA_BREAKTHROUGH_001 contract. (4) Operator's "are we properly mirroring in Supabase and pushing to git? has the UAT been performed?" framing answered fully + atomically: yes / yes / yes for the doctrine surface that motivated the question, with a single commit that closes 5 hardening dimensions simultaneously. (5) The "ERP in prod?" question is now correctly forward-chartered to a separate initiative (HLK-ERP surfacing P4 of `I-NN-OUTPUT-ARCHITECTURE`) rather than rushed at Wave L scope; this preserves the inline-ratify discipline of *charter the right initiative for the right work* over the *do everything in one wave* anti-pattern that I86 §1.6 warned against.
