---
initiative_id: INIT-OPENCLAW_AKOS-76
title: I76 Madeira Elevation operator-interaction quality at Cursor-grade
status: active
authored: 2026-05-18
last_review: 2026-05-18
inception_decision_id: D-IH-76-A
...
authoritative_plan: docs/wip/planning/76-madeira-elevation/master-roadmap.md
---
...
```mermaid
flowchart LR
    P0[P0 Charter + scope-overlap tracker mint]
    P1[P1 Mode parity baseline]
    P2[P2 Tool catalog + per-mode RBAC]
    P3[P3 Operator UX + persistence + personality]
    P4[P4 AICs implementation per F5 hybrid]
    P5[P5 Five-engagement UAT]
    P6[P6 Closure PAUSE]
    Tracker[I11 I13 I17 scope-overlap-tracker]
...
flowchart TB
    subgraph operatorLayer [Operator interface layer]
        Cursor[Cursor IDE session]
        CLI[Future CLI / future hosted]
    end
    subgraph methodLayer [MADEIRA method layer]
        LCL[LOGIC_CHANGE_LOG awareness]
        DR[DECISION_REGISTER awareness]
        BVF[BRAND_VOICE_FOUNDATION rails]
        DoD[Agentic-as-DoD recursive doctrine]
    end
    subgraph runtimeLayer [Agent runtime layer]
        Modes[5 modes Ask Plan Agent Debug Methodology]
        Tools[Tool catalog with per-mode RBAC]
        Persist[Persistence policy methodology-scoped read-only default]
        Persona[Personality config neutral plus operator-voice flag]
    end
    subgraph aicLayer [AICs F5 hybrid per-task]
        Dispatch[F5 dispatcher per-task picks]
        F1[F1 supervised sub-agents]
        F2[F2 peer companions]
        F3[F3 ad-hoc dispatchers]
        F4[F4 single-agent rich-tools]
    end
    subgraph auditLayer [Audit + observability]
        Lf[Langfuse trace per session]
        ReviewStamp[review-stamp dimension I71 P4]
    end
...
```mermaid
flowchart LR
    subgraph ask [Ask mode read-only]
        AskTools[Read Grep Glob WebSearch]
        AskRBAC[No write no shell no MCP-write]
    end
    subgraph plan [Plan mode]
        PlanTools[Read plus AskQuestion plus plan-file write]
        PlanRBAC[Write only to plans dir]
    end
    subgraph agent [Agent mode]
        AgentTools[All tools plus Shell plus MCPs plus subagents]
        AgentRBAC[Full per atomic-phase discipline]
    end
    subgraph debug [Debug mode]
        DebugTools[Read plus observability plus hypothesis-ladder]
        DebugRBAC[No write no commit explicit hypothesis log]
    end
    subgraph methodology [Methodology mode NEW]
        MethTools[Read plus LOGIC_CHANGE_LOG candidate plus DR candidate plus BVF check]
        MethRBAC[Methodology-checkpoint discipline]
    end
