...
- **2026-05-03 (D-IH-50-D execution)** — Telemetry promotion run completed. `py scripts/promote_telemetry_to_scenario.py --since-days 60` scanned the operator's local feed (`~/.openclaw/telemetry/madeira-answer-quality-2026-04-03.jsonl`, 30 records, 32 KB) and emitted **9 proposals** clustered by `(route_kind × residual_flag)`. Top three by `match_count` × leverage selected for operator merge per cycle cap of 1-3 (D-IH-50-D default).

  **G-50-2 per-row operator merges (2026-05-03):**

  - **`SCN-OP-TP-001-V1`** — sourced from proposal `TP-ADM-MISSIN-007` (7 matches; cluster `admin × missing_explicit_escalation` + `admin_brainstorm_drift`). Persona `OPERATOR`, skill `SKILL-MADEIRA-LOOKUP-V1`, class `lookup`, difficulty `moderate`, expected route `admin_escalate`, outcome `ESCALATE`, lifecycle `scaffold`, priority_score `9.000000`. **Operator merge approved 2026-05-03**: cluster represents a recurrent admin-restructure prompt that shipped without a clarifying-escalation gate; row pins the expected behavior for next cycle's calibration.
  - **`SCN-OP-TP-002-V1`** — sourced from proposal `TP-NON-NON_TO-005` (5 matches; cluster `non_tool_answer × non_tool_answer`). Persona `OPERATOR`, skill `SKILL-MADEIRA-LOOKUP-V1`, class `lookup`, difficulty `moderate`, expected route `hlk_lookup`, outcome `GROUND`, lifecycle `scaffold`, priority_score `9.000000`. **Operator merge approved 2026-05-03**: cluster represents factual role-lookup queries (e.g., "Who is the CTO?") answered without a tool call; row pins that hlk_lookup must run.
  - **`SCN-OP-TP-003-V1`** — sourced from proposal `TP-NON-INTERN-004` (4 matches; cluster `non_tool_answer × internal_tool_leak`). Persona `OPERATOR`, skill `SKILL-MADEIRA-LOOKUP-V1`, class `lookup`, difficulty `moderate`, expected route `hlk_lookup`, outcome `REFUSE`, lifecycle `scaffold`, priority_score `6.000000`. **Operator merge approved 2026-05-03**: cluster represents lookup answers that leak internal tool/scaffold names; row pins the brand-jargon-fail-safe (forbidden_keywords includes `internal_tool_leak`, `hlk_search`, `_dtp_`, `openclaw`).

  **6 proposals NOT merged this cycle** (cycle cap honored): `TP-ADM-INTERN-003` (3 matches; admin + internal_tool_leak), `TP-HLK-INTERN-002 ×2` (hlk_search_explicit and hlk_search_fallback variants; 2 matches each), and 3 lower-frequency clusters. Carried forward as future-cycle candidates; no automatic re-merge — re-emit + re-triage required.

  **Verification:** `validate_persona_scenario_registry.py` PASS (rows 326 → **329**); `validate_hlk.py` PASS overall; difficulty distribution moderate count 133 → **136** (still inside D-IH-47-C 40/40/10/10 tolerance); priority scores match the I49 deterministic formula (`recalculate_persona_scenario_priorities.py --dry-run` reports 0 changes).
