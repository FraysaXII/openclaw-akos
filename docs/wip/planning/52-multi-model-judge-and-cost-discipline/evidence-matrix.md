...
| E11 | I52/P6 wires three new dossier surfaces: Section 03 madeira-flavor judge-axis fail summary (per-axis FAIL count + worst axis + roster); Section 04 worst-axis cross-reference; Section 08 endpoint cost subsection with per-status operator-action one-liners. 22 new tests + 299 dossier tests pass; renders honest "not yet" states when judges/probes are absent. | [`reports/p6-dossier-surface-2026-05-03.md`](reports/p6-dossier-surface-2026-05-03.md) | Surface ready for first multi-judge + endpoint-probe live run (P8 closure target); answers "what happens if we deploy a huge model on RunPod?" in <1 click via Section 8 operator-action column |
| E12 | I52/P7 activates CI: AKOS_JUDGE_ROSTER + MAX_JUDGE_USD_PER_RUN env wired into Tier-B matrix (G-52-3); new endpoint-envelope-gate job runs probe + alarm after matrix (G-52-4); 16 new workflow-shape tests + 47 total tier-b tests pass; YAML round-trips; calibration-drift-gate (I51 P5) preserved. | [`reports/p7-ci-activation-2026-05-03.md`](reports/p7-ci-activation-2026-05-03.md) | OPS-47-8 architecturally closed; CI surface ready for first multi-judge live run via `gh workflow run eval-tier-b.yml -f judge_roster=…`; first real endpoint probe deferred to OPS-52-2 (one-line swap from --stub to --runs-jsonl) |
