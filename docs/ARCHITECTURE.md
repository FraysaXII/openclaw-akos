...
| `akos/peopl_engagement_runbooks.py` | I73 People Operations engagement runbooks: shared `log_runbook_pairing()` helper for thin `scripts/peopl_engagement*.py` CLIs that embed `process_list.csv` `item_id` strings for `validate_process_list_pairing.py` discovery |
