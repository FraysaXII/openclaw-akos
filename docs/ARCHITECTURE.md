...
| `scripts/validate_program_id_consistency.py` | Cross-asset `program_id` FK consistency (Initiative 23 / P3): asserts every `program_id` reference in `GOI_POI_REGISTER.csv`, `ADVISER_OPEN_QUESTIONS.csv`, `FOUNDER_FILED_INSTRUMENTS.csv`, `_assets/<plane>/<program_id>/<topic_id>/`, and vault `programs/<program_id>/` resolves to a row in `PROGRAM_REGISTRY.csv`. Called from `validate_hlk.py`. SKIPs gracefully when the registry is absent. |
