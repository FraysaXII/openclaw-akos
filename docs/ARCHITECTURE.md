...
| `akos/peopl_engagement_runbooks.py` | I73 People Operations engagement runbooks: shared `log_runbook_pairing()` helper for thin **`scripts/peopl_engagement*.py`** CLIs (registry maintenance, intake classification, lifecycle routing, outsourced SOC review, percentage payout, investor round review, apprentice curriculum assignment) plus **`scripts/peopl_recruiter_onboarding_checklist_stub.py`** (recruiter brief checklist; `tbi_peopl_dtp_recruiter_onboarding_001`) plus sibling **`scripts/check_ethics_learning_review_due.py`** (quarterly Ethics + Learning advisory checklist; `hol_peopl_dtp_316`) that embed `process_list.csv` `item_id` strings for `validate_process_list_pairing.py` discovery; subprocess smoke in `tests/test_peopl_engagement_scripts_smoke.py` + `tests/test_peopl_recruiter_onboarding_checklist_stub_smoke.py` + `tests/test_check_ethics_learning_review_due_smoke.py` |
| `akos/brand_voice_register.py` | I71 P1 Pack A1 + I71 P2 Packs A2 + A3 + Addition 11 + **I71 P5 Pack A4**: Pydantic chassis for the brand voice + Gantt + multilingual + localised-format + render-ownership validator family. **P1 models (16):** `TicFamily`, `RegisterRule`, `AudienceQuadrant`, `RegisterToken`, `AudienceClass`, `BoundaryRule`, `SubMarkTier`, `VoicePersona`, `EngagementType`, `LLMToneTell`, `LocaleLeakRule`, `CobrandRule`, `TrackRecordRule`, `BrandAbbreviationRule`, `ArchetypeViolation`, `BrandedHouseViolation`, `BrandVoiceRegisterPack` (composite). **P2 additive models (7 + 2 sibling packs):** `GanttConfidenceRule`, `AudienceQuadrantRule`, `LocaleSuffixRule`, `ReadmeTriadRule`, `NumberFormatRule`, `CurrencyFormatRule`, `DateFormatRule`, `BrandGanttPack` (Pack A2 + Addition 11 surfaces), `BrandMultilingualPack` (Pack A3 surfaces). **P5 additive models (2):** `RenderOwnershipRule`, `RenderOwnershipPack` (Pack A4 surfaces; advisory default severity per WORKSPACE_BLUEPRINT §16 discipline). **P1 parsers:** `parse_tic_families_from_canonical` (BRAND_COPYWRITING_DISCIPLINE §2), `parse_english_register_rules` (BRAND_ENGLISH_PATTERNS §5.1), `parse_llm_tone_tells` (BRAND_LLM_TONE_TELLS §3-§7), `parse_register_matrix` (BRAND_REGISTER_MATRIX), `parse_audience_quadrants` (BRAND_GANTT_DISCIPLINE §2), `parse_register_pack_yaml`. **P2 additive parsers (7):** `parse_gantt_confidence_rules` (BRAND_GANTT_DISCIPLINE §4 5-level ladder), `parse_audience_quadrant_rules` (BRAND_GANTT_DISCIPLINE §2 4-quadrant matrix), `parse_gantt_pack_yaml`, `parse_locale_suffix_rules` (BRAND_MULTILINGUAL_CONTRACT §2), `parse_readme_triad_rules` (BRAND_MULTILINGUAL_CONTRACT §2 + D-IH-70-P), `parse_multilingual_pack_yaml`, `parse_localised_format_rules` (BRAND_LOCALISED_FORMATS.md per-locale number/currency/date). **P5 additive parsers (2):** `parse_render_ownership_rules` (WORKSPACE_BLUEPRINT §16 canonical 9-row matrix; deck/proposal/tarification/gantt/dossier/counterparty_brief/objections/press/advisor_email), `parse_render_ownership_pack_yaml`. Strict-day-1 default per D-IH-71-F (P1) + Round 3 brand-DNA Layers 5-9 per D-IH-71-K (P1) + chassis-extension verdict per D-IH-71-S (P5; chassis grew P1 ~517 LOC → P2 ~1193 LOC → P5 ~1340 LOC; under 1800 LOC re-evaluation threshold). P5 additive contract proven by 28/28 P1 + Pack A2+A3 test regression. Consumed by `scripts/validate_brand_voice_register.py` (P1) + `scripts/validate_brand_gantt_confidence.py` (P2 Pack A2) + `scripts/validate_brand_multilingual.py` (P2 Pack A3) + `scripts/generate_vale_styles.py` (P2 Tier 1 Vale sibling generator) + `scripts/validate_render_ownership.py` (P5 Pack A4). |
...
People **[Compliance vs Ethics boundary](docs/references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/PEOPLE_COMPLIANCE_VS_ETHICS_BOUNDARY.md)** (I73 P6) routes ambiguous `hol_peopl_*` stewardship across Compliance / Ethics / Learning / People Operations with explicit jointly-owned disclosure posture defaults (**C-73-5**).

**KB human-readability overlays** ([`KB_HUMAN_READABILITY_CHARTER.md`](docs/references/hlk/v3.0/Admin/O5-1/Operations/PMO/canonicals/KB_HUMAN_READABILITY_CHARTER.md), I73 P7) map four persona presets to [`ENGAGEMENT_MODEL_REGISTRY.csv`](docs/references/hlk/v3.0/Admin/O5-1/People/People%20Operations/canonicals/dimensions/ENGAGEMENT_MODEL_REGISTRY.csv) `engagement_model_id` values (`access_levels.md`-aligned KM trust surfaces; additive overlays on [`HLK_KM_TOPIC_FACT_SOURCE.md`](docs/references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/HLK_KM_TOPIC_FACT_SOURCE.md)). HLK-ERP route contract lives in **[`docs/wip/planning/73-people-operations-and-learning-curriculum/reports/kb-human-readability-erp-route-spec.md`](docs/wip/planning/73-people-operations-and-learning-curriculum/reports/kb-human-readability-erp-route-spec.md)** (cross-repo sibling PR — `hlk-erp` not in this repository).

**Methodology IP minting path** ([`METHODOLOGY_IP_MINTING_PATH.md`](docs/references/hlk/v3.0/Admin/O5-1/Marketing/Brand/canonicals/METHODOLOGY_IP_MINTING_PATH.md), I73 P8): corpus-to-asset lifecycle + **Holistika default vessel** vs personal-name criteria matrix (**D-IH-73-F**); **consult counsel before filings**; Legal / brand cross-links including [`BRAND_HIERARCHY_AND_TRADEMARK_SCOPE_2026-04.md`](docs/references/hlk/v3.0/Admin/O5-1/People/Legal/canonicals/BRAND_HIERARCHY_AND_TRADEMARK_SCOPE_2026-04.md) + [`LOGIC_CHANGE_LOG.md`](docs/references/hlk/v3.0/Admin/O5-1/People/canonicals/LOGIC_CHANGE_LOG.md).

The current HLK registry baseline exposes 11 projects and **1,160** registered items, including founder-governance processes under Legal, Finance, and Compliance for entity formation, founder funding, startup-certification readiness, and trademark/naming control, plus the GTM/Trello-harmonized operating tree merged from the MADEIRA planning candidate (Tier C backlog rows excluded). Initiative **49** registers the MADEIRA **quality and verdict management** workstream (`env_tech_ws_madeira_quality`) plus six **`env_tech_dtp_madeira_*`** governance processes under MADEIRA Platform. Initiative **14** added **Holistika internal GTM** process rows (`holistika_gtm_dtp_*`) via `scripts/merge_process_list_tranche.py`. **API lifecycle** workstream **`env_tech_ws_api_1`** under **HLK Infrastructure and DevOPS** (`env_tech_prj_4`) registers portfolio governance (`env_tech_dtp_306`–`312`) and **component matrix maintenance** (`env_tech_dtp_313`). **Initiative 18** extends **FINOPS counterparty economics** (`thi_finan_ws_4`) with register maintenance and related Finance processes (`thi_finan_dtp_303`–`307`, **`thi_finan_dtp_309`**), **Stripe FDW stewardship** (`thi_finan_dtp_308` under Revenue Operations), canonical **`FINOPS_COUNTERPARTY_REGISTER.csv`**, migration DDL for **`compliance.finops_counterparty_register_mirror`** (cutover from the Initiative 16 vendor mirror), and **`holistika_ops.stripe_customer_link.finops_counterparty_id`**. GTM leaves sit under English **cluster** process rows (`gtm_cl_*`) between workstreams and tasks; see `scripts/refine_gtm_process_hierarchy.py`. Optional **program** workstreams (`hlk_prog_*`) may sit between selected projects and their MADEIRA / Think Big GTM workstreams; see `scripts/migrate_process_list_program_layer.py`. The same CSV may include **`item_parent_1_id`** and **`item_parent_2_id`** (stable parent `item_id` pointers) dual-written with **`item_parent_1`** / **`item_parent_2`** names; see `scripts/backfill_process_parent_ids.py` and `scripts/validate_hlk.py`.

**Component and service matrix:** Canonical **`docs/references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/techops/COMPONENT_SERVICE_MATRIX.csv`** (CTO chain; relocated 2026-05-22 from the canonicals root per **D-IH-81-G-T5** / I81 P2 Tranche T5; Initiative 22 forward layout `techops/` plane; deprecation alias supported in validators for one initiative cycle) lists systems and services with FK-style links to `baseline_organisation.csv`, `process_list.csv`, and `REPOSITORIES_REGISTRY.md` (`repo_slug`). Validated by `scripts/validate_component_service_matrix.py`, invoked from `scripts/validate_hlk.py` when the file exists. Schema: `COMPONENT_SERVICE_FIELDNAMES` in `akos/hlk_component_service_csv.py`.

**FINOPS counterparty register:** Canonical **`docs/references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/finops/FINOPS_COUNTERPARTY_REGISTER.csv`** (Business Controller / CFO chain; moved to `finops/` 2026-05-23 per I81 P2 T1 / D-IH-81-Q under D-IH-81-G umbrella, per Initiative 22 forward layout convention; deprecation alias supported in `validate_finops_counterparty_register.py`, `sync_compliance_mirrors_from_csv.py`, `validate_review_stamps.py` for one initiative cycle) holds counterparty **metadata** only—vendors, customers, and partners (no amounts). Validated by `scripts/validate_finops_counterparty_register.py`, invoked from `validate_hlk.py` when the file exists. Supabase mirror: **`compliance.finops_counterparty_register_mirror`** (mirror table name unchanged); DDL SSOT in [`supabase/migrations/`](../supabase/migrations/) (see `20260423014144_i18_finops_counterparty_mirror_cutover.sql`, which migrates from `finops_vendor_register_mirror` when present); upserts via `scripts/sync_compliance_mirrors_from_csv.py` (`--finops-counterparty-register-only` or full mirror profile **`py scripts/verify.py compliance_mirror_emit`**) after DDL exists on the target DB. Maintenance: `docs/references/hlk/v3.0/Admin/O5-1/Finance/Business Controller/SOP-HLK_FINOPS_COUNTERPARTY_REGISTER_MAINTENANCE_001.md`.
...
**FINOPS writer substrate (I81 P2 Bundle B-2; 2026-05-24):** The Bundle B-2 stack stands up the first end-to-end FINOPS pipeline in Holistika MasterData per `D-IH-81-V` (B-2a substrate) + `D-IH-81-W` (B-2b executable layer) + `D-IH-81-X` (B-2c data + governance). Architecture R1..R5 (ratified 2026-05-23 in [`docs/wip/planning/81-vault-integrity-layout-milestones-retrofit/reports/p2-bundle-b2-architecture-2026-05-23.md`](wip/planning/81-vault-integrity-layout-milestones-retrofit/reports/p2-bundle-b2-architecture-2026-05-23.md)):

- **R1-a Engagement-model router**: `compliance.engagement_model_registry_mirror` carries a 17th column `counterparty_resolution_strategy` (`NOT NULL` + `CHECK` enum: `stripe_customer_link_lookup` / `metadata_engagement_id` / `metadata_billing_plane` / `rpp_payout_attribution` / `manual_review`). Pydantic SSOT in [`akos/hlk_engagement_model_csv.py`](../akos/hlk_engagement_model_csv.py) (`VALID_COUNTERPARTY_RESOLUTION_STRATEGIES` frozenset + `Literal` type on `EngagementModelRow`). The TS-side resolver in [`supabase/functions/_shared/finops/counterparty_resolver.ts`](../supabase/functions/_shared/finops/counterparty_resolver.ts) materialises the router: given a Stripe event + engagement-model row, returns a resolved `counterparty_id` slug or `manual_review`.
- **R2-a ECB FX cache**: `holistika_ops.fx_rate_cache` stores daily ECB rates (USD/EUR + GBP/EUR + CHF/EUR + EUR/EUR identity); refreshed by Edge Function [`supabase/functions/fx-rate-cache-refresh/index.ts`](../supabase/functions/fx-rate-cache-refresh/index.ts) on cron `30 15 * * *` (daily 15:30 UTC = 16:30 CET, after ECB publishes ~16:00 CET). Pydantic in [`akos/hlk_fx_rate.py`](../akos/hlk_fx_rate.py) with ECB-business-day arithmetic (weekends/holidays roll back to last business day). FX snapshot computed at write-time in [`supabase/functions/_shared/finops/fx_snapshot.ts`](../supabase/functions/_shared/finops/fx_snapshot.ts) — joins ECB cache + Stripe-supplied `payment_intent.amount_converted` and flags > 5% divergence via `fxRatesDiverge()` → emits `fx_rate_diverged` OPS row.
- **R3-a pgmq DLQ**: Stripe webhook handler enqueues onto `pgmq.finops_writer_queue` (<5s SLA to return 200 to Stripe); worker drains every minute via [`supabase/functions/finops-writer-worker/index.ts`](../supabase/functions/finops-writer-worker/index.ts) on cron `* * * * *` (MAX_BATCH=25; VISIBILITY_TIMEOUT=90s). Failed messages route to `pgmq.finops_writer_dlq` after MAX_RETRIES=5; `checkDlqDepth()` emits `dlq_threshold_exceeded` OPS row (severity=critical) when depth > 10 + cites [`scripts/finops_dlq_drain.py`](../scripts/finops_dlq_drain.py) operator drain runbook. Five `SECURITY DEFINER` RPC wrappers (`pgmq_send_finops_writer` / `pgmq_read_finops_writer` / `pgmq_delete_finops_writer` / `pgmq_archive_finops_writer` / `pgmq_metrics_finops_writer`) gate pgmq access to `service_role` only (anon + PUBLIC EXECUTE revoked via migration `20260524130000_i81_p2_b2b_pgmq_rpc_wrappers_role_lockdown.sql`).
- **R4-a HLK-ERP OPS-row convergence**: All FINOPS pipeline events emit to `compliance.ops_register_mirror` via [`supabase/functions/_shared/finops/ops_register_emit.ts`](../supabase/functions/_shared/finops/ops_register_emit.ts); ERP consumer panel is forward-chartered as OPS-81-7.
- **R5-triple commit shape**: shipped as 3 atomic commits — `15f69b0` (B-2a substrate) + `b9dc656` (B-2b executable) + `21b82e5` (B-2c data + governance) — plus per-commit SHA-backfill hygiene.

Migrations: `20260524000000_i81_p2_b2_finops_writer_substrate.sql` (B-2a DDL: pgmq queues + raw event archive + idempotency-key UNIQUE) + `20260524100000_i81_p2_b2b_pgmq_rpc_wrappers.sql` (B-2b RPC wrappers) + `20260524120000_i81_p2_b2c_engagement_model_resolution_strategy.sql` (B-2c 17th column + CHECK + per-row UPDATE + 3 new rows) + `20260524130000_i81_p2_b2b_pgmq_rpc_wrappers_role_lockdown.sql` (security lockdown) + `20260524005543_i81_p2_b2c_finops_writer_worker_cron_schedule.sql` + `20260524005706_i81_p2_b2c_fx_rate_cache_cron_schedule.sql` (cron schedules). Stripe webhook handler refactored to dispatch pattern at version 6 ([`supabase/functions/stripe-webhook-handler/index.ts`](../supabase/functions/stripe-webhook-handler/index.ts)) — `dispatch/finops_dispatch.ts` is additive; `dispatch/kirbe_holistika_dispatch.ts` preserves the pre-B-2b kirbe + holistika-ops flows verbatim. PostgREST exposed schemas: operator-confirmed `holistika_ops` + `finops` exposed via Supabase Dashboard (Project Settings → API → Exposed schemas) 2026-05-24. Closure UAT: [`docs/wip/planning/81-vault-integrity-layout-milestones-retrofit/reports/i81/p2-bundle-b2-closure-uat-2026-05-24.md`](wip/planning/81-vault-integrity-layout-milestones-retrofit/reports/i81/p2-bundle-b2-closure-uat-2026-05-24.md) (verdict: PASS-WITH-FOLLOWUP; 1 SKIP for Stripe live AT MCP audit deferred to OPS-81-X).
...
- **`advops/ADVISER_ENGAGEMENT_DISCIPLINES.csv`** — small lookup of adviser disciplines (`legal`, `fiscal`, `ip`, `banking`, `certification`, `notary`) joining `baseline_organisation.csv` (`canonical_role`) and `process_list.csv` (`default_process_item_id`). Fieldnames in `akos/hlk_adviser_disciplines_csv.py`; validator `scripts/validate_adviser_disciplines.py`; mirror `compliance.adviser_engagement_disciplines_mirror`. Relocated to `advops/` at I81 P2 T2 (D-IH-81-R under D-IH-81-G umbrella, 2026-05-23).
- **`advops/ADVISER_OPEN_QUESTIONS.csv`** — adviser-facing questions and actions across disciplines and programs (replaces `FOUNDER_OPEN_QUESTIONS_EXTERNAL_COUNSEL.md` as SSOT; vault MD becomes a derived per-discipline view). Fieldnames in `akos/hlk_adviser_questions_csv.py`; validator `scripts/validate_adviser_questions.py`; mirror `compliance.adviser_open_questions_mirror`. Relocated to `advops/` at I81 P2 T2 (D-IH-81-R under D-IH-81-G umbrella, 2026-05-23).
- **`advops/FILED_INSTRUMENTS.csv`** — filed legal/fiscal/IP/banking/certification/notary instruments across draft/signed/filed/superseded statuses (replaces `FOUNDER_FILED_INSTRUMENT_REGISTER.md` as SSOT; vault MD becomes a derived per-discipline view). Fieldnames in `akos/hlk_filed_instruments_csv.py`; validator `scripts/validate_filed_instruments.py`; mirror `compliance.filed_instruments_mirror`. Relocated + renamed at I81 P2 T3 (D-IH-81-S under D-IH-81-G umbrella, 2026-05-23) from `FOUNDER_FILED_INSTRUMENTS.csv` at canonicals root; deprecation shims at `akos/hlk_founder_filed_instruments_csv.py` and `scripts/validate_founder_filed_instruments.py` re-export / shell into the renamed modules for one initiative cycle (removal at I81 P9 closure). Supabase mirror table renamed via `supabase/migrations/20260523000000_i81_p2_t3_alter_filed_instruments_mirror.sql`.
...
**Workspace blueprint (engagement-folder doctrine, P13):** The canonical engagement-folder shape across the four persistence channels (git, Drive, SQL mirror, HLK-ERP) lives at [`docs/references/hlk/v3.0/Admin/O5-1/Operations/PMO/WORKSPACE_BLUEPRINT_HOLISTIKA.md`](references/hlk/v3.0/Admin/O5-1/Operations/PMO/WORKSPACE_BLUEPRINT_HOLISTIKA.md). It codifies the sub-mark functional split (Holistika provides canon / intel / SOPs; HLK Tech Lab provides tech; Think Big is the engagement arm), the five engagement types mapped to two physical roots (`Think Big/Clients/` outbound + `Think Big/Advisers/` inbound) and three semantic directions (outbound-external, outbound-internal via reserved `internal-` slug, inbound), the file-tracking policy (branded PDFs tracked; markdown sidecars ignored), and the role-owner-canonical vs `compliance/`-tightened-mirror contract (canonical SOP wins on conflict; mirror is the machine-readable companion; future fusion deferred). All new engagement folders conform to the per-root skeletons; templates live at `Think Big/Clients/_engagement-template/` (outbound) and `Think Big/Advisers/_engagement-template/` (inbound).
...
1. **Schema (DDL)** — Canonical forward migrations live in [`supabase/migrations/`](../supabase/migrations/) and ship with the [Supabase CLI](https://supabase.com/docs/guides/deployment/database-migrations), pinned via `npx supabase` (canonical contract in [`SOP-HLK_TOOLING_STANDARDS_001.md`](references/hlk/v3.0/Admin/O5-1/Tech/System%20Owner/SOP-HLK_TOOLING_STANDARDS_001.md) §3.1): `npx supabase db push` (or CI). Staging DDL under `scripts/sql/*_staging/` is a **proposal** until copied into a new migration after [`operator-sql-gate.md`](wip/planning/14-holistika-internal-gtm-mops/reports/operator-sql-gate.md) approval. **Ledger parity:** migration **filename timestamps** must match remote `schema_migrations`; run `npx supabase migration list` after `link` and reconcile (rename or `npx supabase migration repair`) before `npx supabase db push` when drift appears—see [`supabase/migrations/README.md`](../supabase/migrations/README.md).
2. **Compliance mirrors (derived data)** — Git-canonical CSVs under `docs/references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/` remain SSOT; `compliance.*_mirror` tables are refreshed from [`scripts/sync_compliance_mirrors_from_csv.py`](../scripts/sync_compliance_mirrors_from_csv.py). **Do not** commit megabyte upserts as migrations.
...
| `scripts/validate_filed_instruments.py` | Validate `advops/FILED_INSTRUMENTS.csv` (Initiative 21; renamed I81 P2 T3 from `validate_founder_filed_instruments.py` — legacy script remains as deprecation shim for one cycle); called from `validate_hlk.py` |
| `scripts/validate_program_registry.py` | Validate `dimensions/PROGRAM_REGISTRY.csv` (Initiative 23 / P1): cycle detection on `parent_program_id` + `consumes_program_ids` (DAG); `program_code` uniqueness + pattern; FK to `baseline_organisation.csv`, `process_list.csv`; enums; `--allow-forward-references` for tranche-window relaxation. Called from `validate_hlk.py`. |
| `scripts/validate_program_id_consistency.py` | Cross-asset `program_id` FK consistency (Initiative 23 / P3): asserts every `program_id` reference in `GOI_POI_REGISTER.csv`, `advops/ADVISER_OPEN_QUESTIONS.csv`, `advops/FILED_INSTRUMENTS.csv`, `_assets/<plane>/<program_id>/<topic_id>/`, and vault `programs/<program_id>/` resolves to a row in `PROGRAM_REGISTRY.csv`. Called from `validate_hlk.py`. SKIPs gracefully when the registry is absent. |
| `scripts/probe_compliance_mirror_drift.py` | Operator-pasted compliance mirror drift probe (Initiative 23 / P4): `--emit-sql` prints a `UNION ALL SELECT` covering all 8 mirrors; operator pastes JSON result into `artifacts/probes/mirror-drift-<YYYYMMDD>.json` (gitignored); `--verify` (default) compares live counts against canonical CSV row counts and reports row-by-row PASS/FAIL. SKIPs gracefully when no fresh artifact exists. Verify profile `compliance_mirror_drift_probe`. |
...
| `scripts/sync_compliance_mirrors_from_csv.py` | Emit `INSERT … ON CONFLICT` SQL for `compliance.process_list_mirror` / `baseline_organisation_mirror` / `finops_counterparty_register_mirror` / `goipoi_register_mirror` / `adviser_engagement_disciplines_mirror` / `adviser_open_questions_mirror` / `filed_instruments_mirror` from git CSVs (`--count-only`, `--output`, **`--finops-counterparty-register-only`**, **`--goipoi-register-only`**, **`--adviser-disciplines-only`**, **`--adviser-questions-only`**, **`--founder-filed-instruments-only`** — CLI flag name retained for one cycle; emits to renamed table); run on DB only after approved DDL (see `sql-proposal-stack-20260417.md`, Initiative 21 staging in `scripts/sql/i21_phase1_staging/`). Mirror table renamed at I81 P2 T3 (D-IH-81-S; `compliance.founder_filed_instruments_mirror` -> `compliance.filed_instruments_mirror`). |
...
| `scripts/dedupe_ambiguous_process_item_names.py` | Repair: rename a fixed set of rows (by `item_id`) so every `item_name` is unique; **`--report`** lists collisions; **`--suggest`** prints starter `RENAMES_BY_ITEM_ID` lines; **`--write`** applies map + `resolve_all_parent_ids` |
| `scripts/check_process_list_header.py` | Fail if `process_list.csv` header ≠ `PROCESS_LIST_FIELDNAMES` (fork / legacy column drift) |
...
| HLK Registry         | `akos/hlk.py`, `akos/hlk_graph_model.py`, `akos/hlk_neo4j.py`, `akos/hlk_vault_links.py`, `akos/hlk_substrate_registry_csv.py` (I84 P3 substrate doctrine SSOT), `akos/hlk_canonical_articulation.py` (I95 HCAM entity + relationship registries), `scripts/hlk_mcp_server.py`, `scripts/hlk_graph_mcp_server.py`, `scripts/sync_hlk_neo4j.py`, `scripts/validate_substrate_registry.py`, `scripts/validate_canonical_articulation.py` | CSV registry lookups + optional Neo4j mirrored graph + vault link validation + substrate doctrine canonical (per `D-IH-84-F` 18-column schema covering OpenClaw / Cursor SDK / hybrid / LangGraph / CrewAI / Letta / etc.; paired with Research-area [`SUBSTRATE_LANDSCAPE_DOCTRINE.md`](references/hlk/v3.0/Admin/O5-1/Research/Methodology/canonicals/SUBSTRATE_LANDSCAPE_DOCTRINE.md) at status:review) | MCP + REST `/hlk/graph/*`; `PRECEDENCE.md` classifies Neo4j as mirrored; HCAM articulation CSVs (`ENTITY_CATALOG`, `CANONICAL_RELATIONSHIP_REGISTRY`) are **T1 git + T3 graph projection only** (no compliance mirror per D-IH-95-B); indexed in `CANONICAL_REGISTRY.csv` + [`CANONICAL_GOVERNANCE_REGISTRY.csv`](references/hlk/v3.0/Admin/O5-1/People/Compliance/canonicals/dimensions/CANONICAL_GOVERNANCE_REGISTRY.csv) (P95-GOV-1/2); `validate_hlk_vault_links.py` + `validate_substrate_registry.py` + `validate_canonical_governance_registry.py` in release gate |
| HLK Dimension Registries (28 CSVs under `compliance/canonicals/dimensions/`) | `AIC_CAPABILITY_IMPLEMENTATION_MATRIX.csv`, `AIC_REGISTRY.csv`, `ARTIFACT_CLASS_REGISTRY.csv`, `AUDIENCE_REGISTRY.csv`, `BUILDOUT_BACKLOG.csv`, `CANONICAL_GOVERNANCE_REGISTRY.csv`, `CAPABILITY_CONFIDENCE_REGISTRY.csv`, `CAPABILITY_REGISTRY.csv`, `CHANNEL_TOUCHPOINT_REGISTRY.csv`, `COMPONENT_PRIMITIVE_REGISTRY.csv`, `COUNTRY_WORK_CALENDAR.csv`, `ENGAGEMENT_REGISTRY.csv`, `GOI_POI_REGISTER.csv`, `KNOWLEDGE_PAIRING_REGISTRY.csv`, `MADEIRA_AIC_PER_TASK_REGISTRY.csv`, `OUTPUT_TYPE_REGISTRY.csv`, `PEOPLE_DESIGN_PATTERN_REGISTRY.csv`, `PERSONA_REGISTRY.csv`, `PERSONA_SCENARIO_REGISTRY.csv`, `POC_TO_COMMERCIAL_MAP.csv`, `POLICY_REGISTER.csv`, `PROGRAM_REGISTRY.csv`, `SKILL_REGISTRY.csv`, `SOURCING_REGISTER.csv`, `SUBSTRATE_REGISTRY.csv`, `TOPIC_REGISTRY.csv`, `TOUCHPOINT_KIT_CELL_REGISTRY.csv`, `USE_CASE_ARCHIVE.csv` | Per-axis canonical CSV registries that join across personas / audiences / channels / artefacts / outputs / capabilities / engagements / programs / topics. Each carries Pydantic SSOT under `akos/hlk_<name>_csv.py`, paired validator under `scripts/validate_<name>.py`, Postgres mirror under `compliance.<name>_mirror`, and entry in `PRECEDENCE.md`. Authoritative inventory is `PRECEDENCE.md` (canonicals table) + `IDX-02-PRECEDENCE-CSV-COVERAGE` baseline-index probe per `INDEX_INTEGRITY_DISCIPLINE.md` (I86 Wave N P3; D-IH-86-CD). | Validated by `validate_<name>.py` invoked from `validate_hlk.py`; mirrored to Postgres via `sync_compliance_mirrors_from_csv.py`; FK-resolved across siblings; coverage gate by `scripts/baseline_index_sweep.py` IDX-06 probe. |
