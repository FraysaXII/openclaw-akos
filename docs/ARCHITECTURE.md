...
| HLK Registry         | `akos/hlk.py`, `akos/hlk_graph_model.py`, `akos/hlk_neo4j.py`, `akos/hlk_vault_links.py`, `akos/hlk_substrate_registry_csv.py` (I84 P3 substrate doctrine SSOT), `scripts/hlk_mcp_server.py`, `scripts/hlk_graph_mcp_server.py`, `scripts/sync_hlk_neo4j.py`, `scripts/validate_substrate_registry.py` | CSV registry lookups + optional Neo4j mirrored graph + vault link validation + substrate doctrine canonical (per `D-IH-84-F` 18-column schema covering OpenClaw / Cursor SDK / hybrid / LangGraph / CrewAI / Letta / etc.; paired with Research-area [`SUBSTRATE_LANDSCAPE_DOCTRINE.md`](references/hlk/v3.0/Admin/O5-1/Research/Methodology/canonicals/SUBSTRATE_LANDSCAPE_DOCTRINE.md) at status:review) | MCP + REST `/hlk/graph/*`; `PRECEDENCE.md` classifies Neo4j as mirrored; `validate_hlk_vault_links.py` + `validate_substrate_registry.py` in release gate |
