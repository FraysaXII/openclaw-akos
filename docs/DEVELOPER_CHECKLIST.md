...
| 7-i73 | `py scripts/validate_engagement_model_registry.py` | **Initiative 73 P1**: standalone validator for the new `ENGAGEMENT_MODEL_REGISTRY.csv` (sibling dimension at `People/People Operations/canonicals/dimensions/`). Pydantic SSOT `akos.hlk_engagement_model_csv.ENGAGEMENT_MODEL_FIELDNAMES` enforces 16-column schema + 7-class taxonomy completeness + per-row enum membership (retribution_pattern / soc_posture / ip_clause_class / knowledge_access_level / payment_cadence / status). FK to `baseline_organisation.csv` Data Owner = `People Operations Lead`. Verification profile: `engagement_model_registry_smoke`. |
