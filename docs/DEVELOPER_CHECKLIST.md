...
| 7 | `py scripts/validate_hlk.py` | HLK canonical vault integrity — includes **`COMPONENT_SERVICE_MATRIX.csv`**, **`FINOPS_COUNTERPARTY_REGISTER.csv`**, **`ENGAGEMENT_MODEL_REGISTRY.csv`** (I73 P1 sibling dimension per D-IH-73-C; 7-class engagement-model taxonomy), and other canonical CSV checks when those files exist (also run by release gate) |
| 7-i73 | `py scripts/validate_engagement_model_registry.py` | **Initiative 73 P1 + I81 P2 B-2c extension**: standalone validator for `ENGAGEMENT_MODEL_REGISTRY.csv`. Pydantic SSOT `akos.hlk_engagement_model_csv.ENGAGEMENT_MODEL_FIELDNAMES` enforces **17-column** schema (B-2c extension 2026-05-24 per D-IH-81-X added `counterparty_resolution_strategy` per-row enum membership: `stripe_customer_link_lookup` / `metadata_engagement_id` / `metadata_billing_plane` / `rpp_payout_attribution` / `manual_review`) + 10-class taxonomy completeness (7 existing + 3 new: `saas_subscription` active, `rpp_vendor` planned, `one_off_invoice` planned) + per-row enum membership (retribution_pattern / soc_posture / ip_clause_class / knowledge_access_level / payment_cadence / status). FK to `baseline_organisation.csv` Data Owner = `People Operations Manager`. Verification profile: `engagement_model_registry_smoke`. |
| 7-i81-b2a | `py scripts/validate_finops_ledger.py` | **Initiative 81 P2 Bundle B-2a INFO advisory** (D-IH-81-V; promotes to FAIL per D-IH-81-W closure when first live Stripe `charge_succeeded` round-trip succeeds): exercises `akos.hlk_finops_ledger` Pydantic `RegisteredFactRow` + 4-strategy counterparty resolution ladder + ECB 4-tier FX fallback + 0.5% Stripe-vs-ECB divergence detector + `akos.hlk_ops_register_emit` RICE-scored row emit against synthetic Stripe-event fact stream that FK-resolves to `FINOPS_COUNTERPARTY_REGISTER` `finops_*` slugs. Wired into `release-gate.py` as INFO advisory `run_finops_ledger_validation`. Verification profile: `validate_finops_ledger_self_test`. |
| 7-i81-b2b | `py scripts/finops_dlq_drain.py --self-test` + `py scripts/stripe_audit_metadata.py --self-test` | **Initiative 81 P2 Bundle B-2b INFO advisory** (D-IH-81-W). Two operator runbooks for the R3-a 3-layer retry pattern + R1-a engagement-model-aware router. `finops_dlq_drain.py --self-test` validates `PGMQ_RPC_NAMES` wiring + `DlqEntry` / `DrainOperation` / `DrainSummary` Pydantic models + CLI surface; paired with `supabase/migrations/20260524100000_i81_p2_b2b_pgmq_rpc_wrappers.sql` `SECURITY DEFINER` wrappers. `stripe_audit_metadata.py --self-test` validates `StripeMetadataFinding` / `StripeAuditReport` Pydantic models + classify_customer / classify_subscription predicate functions; surfaces orphan customers + `hlk_billing_plane`-missing subscriptions + `holistika_ops`-without-link rows before live runs. Both wired into `release-gate.py` as INFO advisories (`run_finops_dlq_drain_self_test` + `run_stripe_audit_metadata_self_test`). Per `akos-executable-process-catalog.mdc` Rule 1 AC-AUTOMATION. |
