...
| `GET /hlk/graph/explorer` | **Primary** operator UI (`static/hlk_graph_explorer.html` + vis-network CDN): summary cards; **registry pickers** (areas, roles, projects, children, `/hlk/search`) feed the graph—same SSOT as `/hlk/*`; depth/limit sliders; **node size basis** (children / degree / balanced) and **edge label mode** (auto / always / hover-only) with **sessionStorage** persistence and **re-render without re-fetch**, aligned with the Streamlit explorer’s PARENT_OF / OWNED_BY sizing and edge colour/abbrev conventions; optional API key in **sessionStorage** only; same `Authorization: Bearer` as other `/hlk/*` routes when `AKOS_API_KEY` is set |
