...
| Neo4j `:Program` nodes + program edges | Neo4j Community (Bolt) | `PROGRAM_REGISTRY.csv` --> rebuilt projection via `scripts/sync_hlk_neo4j.py` | **Mirrored read index** for graph queries; **never** authoring surface (Initiative 07 D2). Constraints: `Program.program_id IS UNIQUE`. Range indexes on `lifecycle_status`, `program_code`, `default_plane`. Edges: `:CONSUMES`, `:PRODUCES_FOR`, `:PROGRAM_PARENT_OF`, `:PROGRAM_SUBSUMES`, `:OWNED_BY` (program → role). Edge naming **disambiguated** to coexist with the existing `:PARENT_OF` (process → process). Graceful SKIP when Neo4j unconfigured (Initiative [23](../../../wip/planning/23-hlk-program-registry-and-program-2/master-roadmap.md), D-IH-18) |
