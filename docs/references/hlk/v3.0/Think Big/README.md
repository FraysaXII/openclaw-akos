...
| Folder | Use |
|--------|-----|
| `Clients/` | Stable, client-scoped material (governance, commercials, named engagement folders). |
| `Projects/` | Time-bound or internal project documentation that is **not** the primary home of application source code. |

When a deliverable is **only** in GitHub, do not duplicate the tree here: register the repo in `REPOSITORIES_REGISTRY.md` and link from a topic index.
