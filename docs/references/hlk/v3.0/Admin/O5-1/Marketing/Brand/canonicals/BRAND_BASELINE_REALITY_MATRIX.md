...
- [`RESEARCH_LIFECYCLE_DOCTRINE.md`](../../../../../Research/canonicals/RESEARCH_LIFECYCLE_DOCTRINE.md) §5 — **reciprocal pointer (SHARE/OUTAKE join).** Research's CORPINT information lifecycle ends at SHARE: it authors the finished brief/report in the **internal register**, then hands off to **this dual-register translation** (the Marketing/Brand side) before External-Render delivers the surface. So this matrix is the downstream consumer of Research's intelligence products — the internal→external register flip the SHARE join depends on. Authored under founder-RACI delegation (Research = Responsible; Brand Manager = Consulted; operator = Accountable) per the KB-integrity I81 reciprocal-pointer follow-up.
