...
| **DATAOPS (data quality / pipeline integrity / mirror sync)** | `DATAOPS_DISCIPLINE.md` (Wave M P5 mint per `D-IH-86-BV`; promoted to **active** at I90 P3c per `D-IH-90-AA` with paired SOP `SOP-TECH_DATAOPS_QUALITY_001.md` + runbook `scripts/dataops_quality_check.py` + Pydantic SSOT `akos/hlk_dataops_quality.py`; owned by Tech/Data + System Owner) | active | `compose_DATAOPS(audience, channel, scenario, brand, governance) → data quality bar + pipeline freshness + mirror parity + FDW posture` |
