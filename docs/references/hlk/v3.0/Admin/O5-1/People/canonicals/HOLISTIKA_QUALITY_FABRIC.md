...
pairing with UAT_DISCIPLINE's **classification axis**. Wave R+4 extended to
**15** with RESEARCH_ACTION_DISCIPLINE landing at `active` per `D-IH-86-FF`
(C1.6 mint) as the **research-to-decision governance axis** that operationalises
`akos-applied-research-discipline.mdc` parent rule into an executable
8-stage loop with a 15-field per-source ledger. Wave R+1 P3
