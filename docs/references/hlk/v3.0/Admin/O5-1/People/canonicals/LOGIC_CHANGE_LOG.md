...
| BT-06-dataops-quality-fabric-active | 2026-06-04 | Compliance-artefact quality needs its own executable probe lane — canonical CSV + mirror cadence — distinct from closure UAT (human sign-off) and inter-wave regression (cross-wave drift); the 7-dimension DataOps bar closes that gap as an active Quality Fabric specialty. | v3.1 -> v3.1 (no vault-folder increment; QF specialty activation per SOP-RELEASE_TAXONOMY lane-1 patch) | DATAOPS_DISCIPLINE.md (charter→active); SOP-TECH_DATAOPS_QUALITY_001.md; akos/hlk_dataops_quality.py; scripts/dataops_quality_check.py; process_list env_tech_dtp_dataops_quality_001; CAPABILITY_REGISTRY CAP-HOL-DATAOPS-QUALITY-CHECK-001 | D-IH-90-AA; D-IH-86-BV; RESEARCH_HEAD §6 backfill (DAMA-DMBOK2 2024 revision Ch.13 nine DQ dimensions) |
