...
│   └── FILED_INSTRUMENTS.csv                     (relocated + renamed I81 P2 T3 from canonicals/FOUNDER_FILED_INSTRUMENTS.csv; Pydantic + validator + Supabase mirror table all renamed; deprecation aliases for one cycle)
│
...
| ~~`compliance/FOUNDER_FILED_INSTRUMENTS.csv`~~ **MIGRATED I81 P2 T3** | `compliance/advops/FILED_INSTRUMENTS.csv` | **Relocated + renamed 2026-05-23 per D-IH-81-S under D-IH-81-G umbrella** (T3 atomic-commit; full rename cascade — CSV + Pydantic module + validator script + Supabase mirror table all renamed). Deprecation shims: `akos/hlk_founder_filed_instruments_csv.py` re-exports `FOUNDER_FILED_INSTRUMENTS_FIELDNAMES`; `scripts/validate_founder_filed_instruments.py` shells into renamed validator. Legacy CSV path also supported in `validate_filed_instruments.py`, `sync_compliance_mirrors_from_csv.py`, `validate_review_stamps.py`, `validate_compliance_schema_drift.py`, `probe_compliance_mirror_drift.py`, `validate_hlk.py` dispatcher for **one initiative cycle**; all aliases + shims to be removed at I81 P9 closure. Supabase mirror table renamed via `supabase/migrations/20260523000000_i81_p2_t3_alter_filed_instruments_mirror.sql`. |
