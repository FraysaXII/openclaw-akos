...
- **PMO → RevOps transition**: when I72 ships its P0 charter + first engagement-template promotion (P0 charter ratified 2026-05-14 — see [`docs/wip/planning/72-marketing-area-governance-and-persona-registry-expansion/master-roadmap.md`](../../../../../../wip/planning/72-marketing-area-governance-and-persona-registry-expansion/master-roadmap.md)). PMO retains meta-governance (per §17 3-tier WIP topology); RevOps owns acquisition-driven template promotion. Drift signal: when 3+ engagements consume the same template pattern, RevOps takes over template-iteration responsibility (`D-IH-72-B`). I72 Strand D (Round 5 expansion per `D-IH-72-L`) wires RevOps into Holistika operating system across Finance / Data / Tech / GTM / People / Legal / Research / MADEIRA via three sub-strands: D.1 RevOps Integration Spine (P7 — `governance.engagement_revenue_view` per `D-IH-72-M`); D.2 Process Catalog (P8 — `REVOPS_PROCESS_CATALOG.yaml` + paired SOPs per [`akos-executable-process-catalog.mdc`](../../../../../../../.cursor/rules/akos-executable-process-catalog.mdc) Cursor rule minted at P0 codifying SOP+executable runbook pairing + adapter status metadata + cadence taxonomy + DAMA-DMBOK 2.0 alignment); D.3 Cross-area integration (P9 — 8 adapter registries per Truto / Unified.to / Apideck Normalized Adapter Pattern 2026 + active/inactive lifecycle metadata per `D-IH-72-O` + `D-IH-72-T`). Round 7 path migration per `D-IH-72-Y`: RevOps-owned deliverables move to `Operations/RevOps/canonicals/` (NEW area sub-domain peer to PMO + SMO; RevOps Manager + RevOps Analyst activate at P4 per `D-IH-72-AC`; CRO + COO forward-charted per `D-IH-72-AD`). Round 8 schema cascade per `D-IH-72-AF`: `process_list.csv` extends with 7 new columns at P4 (4 axis FKs + 3 rev value cells) operationalising RevOps' value-mapping core function.
