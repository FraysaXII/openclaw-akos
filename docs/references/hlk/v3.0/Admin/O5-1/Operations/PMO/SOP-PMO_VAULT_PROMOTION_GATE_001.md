...
6. After merge, satisfy the **process list maintenance contract** (unique `item_name`, parent id columns, header shape): see **[SOP-PMO_PROCESS_LIST_CSV_MAINTENANCE_001.md](SOP-PMO_PROCESS_LIST_CSV_MAINTENANCE_001.md)**. In short: run **`py scripts/validate_hlk.py`** until PASS; use **`py scripts/backfill_process_parent_ids.py --write`** when parent ids need recomputation; use **`scripts/dedupe_ambiguous_process_item_names.py`** (`--report` / `--suggest` / `--write`) if duplicate display names appear. Project rows must leave both parent-id columns empty.
