...
| Operations | RevOps | `/operator/operations/revops/engagement-templates/` | ENGAGEMENT_TEMPLATE_REGISTRY canonical CSV + Supabase mirror `compliance.engagement_template_registry_mirror` + 6 seed rows promoted from I70 P2.4 patterns; per-template variant table (proposal / deck / tarification / Gantt / sales-motion fork-pattern) | reserved (I72 P2 — `D-IH-72-F` + `D-IH-72-Y` Round-7 path migration to `Operations/RevOps/canonicals/`) |
| Operations | RevOps | `/operator/operations/revops/engagement-revenue/` | `governance.engagement_revenue_view` joining `finops.registered_fact` + `compliance.engagement_registry_mirror` + `compliance.engagement_template_registry_mirror` + `holistika_ops.stripe_customer_link`; per-engagement booked / billed / recognized rollup per Routine.co RevOps Blueprint terminology; per-template revenue aggregation feeds value-mapping function | reserved (I72 P7 — `D-IH-72-M` option-D both FK + view + Round-7 path migration per `D-IH-72-Y`) |
| Research | Intelligence | `/operator/research/intelligence/intelligenceops-register/` | INTELLIGENCEOPS_REGISTER canonical CSV + Supabase mirror; intelligence-collection structure (`target_id` + `target_class` competitor_intelligence_target/regulator/media/recruiter + `cadence` + `source_type` HUMINT/OSINT/TECHINT/hybrid + `reliability` A-E HUMINT FM 2-22.3 + `output_artifact` + `responsible_role`); cross-links to 3 SOPs (regulator + media + recruiter cross-link) | reserved (I72 P6 — `D-IH-72-H` sibling canonical CSV not GOI_POI col-extension; cross-coordinates with I75 candidate) |
