...
status: active
canonical: true
role_owner: PMO + RevOps Manager (forward; activated at P4)
...
Per `D-IH-72-AC` (Round 7 RevOps activation gating): RevOps Manager + RevOps Analyst flip from gated to active at P4 entry **only after** I71 P5 Pack A4 ships (Pydantic agentic-flow finalization). The CRO row remains `gated_ahead_of_executive_activation`.

**Expansion pattern (per D-IH-72-AM):** when RevOps work volume exceeds the 2-role bandwidth (typically at $5-25M ARR per UnifyGTM RevOps Team Structure 2026 + RevenueTools.io 2026), expand by **adding RevOps Manager seats** (e.g., RevOps Manager - Systems / RevOps Manager - Spine / RevOps Manager - Enablement / RevOps Manager - Data) rather than minting separate role rows for each discipline. This preserves the discipline-holding generalist pattern while permitting headcount growth.
...
- **Activation cadence**: gated. Area minted at P1 (this charter); operational deliverables (template registry, promotion SOP, RevOps Manager/Analyst, spine, catalog, adapters) ship across P2-P9. Full activation at P10 closing UAT.
- **Lifecycle status**: `planned` → `active` at P4 entry (gated on I71 P5 Pack A4).
- **Pre-activation posture**: PMO interim-owns RevOps deliverables until P4 activation per `D-IH-72-AH`. PMO + RevOps Manager transition documented in P4 deliverables.
