...
- **Pydantic + Deno cross-runtime test pattern (I81 P2 Bundle B-2; 2026-05-24):** When a feature spans both the Python authoring layer (Pydantic SSOT in `akos/`) and the Deno runtime layer (Supabase Edge Functions in `supabase/functions/`), tests live on both sides and exercise the same contract:
  - **Python tests** (`tests/test_<feature>.py`) cover the Pydantic model + validator + CSV emit + drift probe + helper functions; use `pytest` + `@pytest.mark.<group>` registration in `scripts/test.py`. Example: `tests/test_validate_engagement_model_registry.py` (26 tests asserting 17-column tuple + 5-strategy enum membership + per-row strategy mapping + invalid-input pair).
  - **Deno tests** (`supabase/functions/_shared/<feature>/<module>.test.ts`) scaffold beside each `.ts` module; run via `deno test` (`b2b-test-b` ratification). They cover the TS-side pure-function contracts (e.g., `counterparty_resolver.ts` strategy ladder; `fx_snapshot.ts` divergence detector; `ops_register_emit.ts` RICE scoring; `stripe_event_logger.ts` idempotency key derivation). Mock Supabase client via dependency injection (functions accept a `SupabaseClient` parameter so tests pass a fake).
  - **Cross-runtime contract drift detector**: the Pydantic frozenset (e.g., `VALID_COUNTERPARTY_RESOLUTION_STRATEGIES`) and the TS-side enum (in the `.ts` resolver) must hold identical members. Add an integration test under `tests/test_<feature>_runtime_parity.py` that reads both sources of truth and asserts equality, OR have one side import-by-codegen from the other (preferred when feasible).
  - **Operator runbook self-test**: every paired runbook script (per `akos-executable-process-catalog.mdc` Rule 1 AC-AUTOMATION) carries a `--self-test` mode that exercises Pydantic models + RPC wiring + CLI surface without external dependencies. Wire into `release-gate.py` as INFO advisory until the live integration round-trip succeeds (then promote to PASS). Example: `scripts/finops_dlq_drain.py --self-test` validates `PGMQ_RPC_NAMES` + `DlqEntry`/`DrainOperation`/`DrainSummary` models + CLI argv.
  - **Closure UAT discipline**: when a Bundle ships Edge Functions to live Supabase, the closure UAT under `docs/wip/planning/<NN>/reports/.../closure-uat-*.md` carries §3.4 browser-evidence-class material via Supabase MCP (`get_logs`, `execute_sql`, `list_edge_functions`) with reproducible commands per `akos-planning-traceability.mdc` §"UAT quality bar" (post-2026-05-19 bar; mandatory for ≥5-phase initiatives + sibling-repo work + canonical-CSV touches). Worked example: `docs/wip/planning/81-vault-integrity-layout-milestones-retrofit/reports/i81/p2-bundle-b2-closure-uat-2026-05-24.md` verdict PASS-WITH-FOLLOWUP.
