...
todos:
  ...
  - id: p3-strand-c1-release-taxonomy
    content: "Phase 3 — Strand C1 release-taxonomy ratification (next-up; closes OPS-71-2). Author docs/references/hlk/v3.0/Admin/O5-1/Tech/System Owner/canonicals/SOP-RELEASE_TAXONOMY_001.md codifying the three release lanes ratified at P0 via D-IH-71-D (methodology major.minor via LOGIC_CHANGE_LOG + D-IH rows / HLK vault folder path stays v3.0 with renames being their own initiatives / openclaw-akos SemVer + CHANGELOG + git tag for repo-level release baselines). Update CHANGELOG.md policy header to point to the SOP. Decide tag-now vs hold for I71 P6 closure at P3 inline-ratify gate (C-71-3 default = hold). WORKSPACE_BLUEPRINT_HOLISTIKA §10 cross-link to the SOP. Mint D-IH-71-P ratification row (renumbered from prior -O slot since Vale row claimed -O at P2). OPS-71-2 closes with closure_decision_id + closed_at."
    status: in_progress
  - id: p4-strand-c2-review-stamp
    content: "Phase 4 — Strand C2 review-stamp dimension (pending; closes OPS-71-3). Pick column-extension vs separate review-stamp table at P4 inline-ratify gate (C-71-4 default = column-extension for already-mirrored tables; separate table for unmirrored canonicals). Author Supabase migration at supabase/migrations/<ts>_i71_p4_review_stamp.sql with NOT VALID + VALIDATE CONSTRAINT pattern per I70 P8.5 precedent. Update affected canonical CSVs (and their akos.* SSOT tuples in akos/hlk_*_csv.py). Extend validate_canonical_registry.py (or sibling) with freshness-window logic. Surface stale rows to OPERATOR_INBOX.md. Reserve ERP panel slot in HLK_ERP_ARCHITECTURE.md §4. Mint D-IH-71-P ratification row. OPS-71-3 closes."
    status: pending
  - id: p5-pack-a4-strand-b-hardening
    content: "Phase 5 — Pack A4 (render ownership) + Strand B hardening (pending; gates I72 P4 RevOps activation). Mint scripts/validate_render_ownership.py enforcing per-deliverable owner coverage from WORKSPACE_BLUEPRINT_HOLISTIKA.md §16 + transition-trigger hints (PMO → RevOps; PMO → HLK Tech Lab) as advisory rows. Mint scripts/check_observability_mcps.py scripted MCP-availability smoke verifying user-sentry + user-langfuse MCPs are reachable; advisory (INFO) row in release-gate, never blocks. Optionally surface dashboard cross-links in WORKSPACE §18. Mint D-IH-71-Q (A4 ratification) + D-IH-71-R (Strand B cardinality ratification). Resolve C-71-5 (observability cardinality: every CI gate has its own row vs generic 'PMO triages') inline-ratify; default every-gate-its-own-row."
    status: pending
  - id: p6-closing-uat
    content: "Phase 6 — Closing UAT + INITIATIVE_REGISTRY closure (pending; closes OPS-71-1). Operator UAT pass per master-roadmap §6 acceptance criteria (validator-pack coverage; AIOps baseline INFO rows; release-taxonomy SOP cross-linked; review-stamp migration applied; freshness-window validator green). Author reports/p71-closing.md. Append CHANGELOG closure entry. INIT-OPENCLAW_AKOS-71 status: closed + closure_decision_id: D-IH-71-CLOSURE + OPS-71-1 closed_at. Optionally push v3.1.0 annotated tag at closure cut if P3 deferred. Forward-charter retention rows for I78 (Tier 2 LLM-judge promote-trigger), Tier 3 writer-facing UX deferral, and any Pack-A5-class follow-ons surfaced during the validator-pack rollout."
    status: pending
isProject: false
---
...
> **Status: active (chartered 2026-05-13).** Three strands: **A** validator rule packs (absorbs I70-deferred P5 / P6 / P7 / P10), **B** AIOps baseline (Sentry + Langfuse via operator MCPs), **C** governance disciplines that I70 forward-chartered (release taxonomy + review-stamp dimension). Sibling to **I68** (consumer-repo CI baseline + InfraMonitor): I71 owns **AKOS-side brand/render validators** + **observability routing** + **release-policy SSOT**, not duplicates of I68's Playwright/Sentry release-format templates.
...
| **P6** | Closing UAT + initiative registry closure | — | pending | — | `OPS-71-1` |
...
| **C-71-3** | Strand C1 tag-now-vs-hold for I71 P6 closure? | pending P3 inline-ratify | hold for P6 closure | TBD at P3 execution. |
| **C-71-4** | Strand C2 column-extension vs separate review-stamp table? | pending P4 inline-ratify | column-extension where table exists; separate table for standalone canonicals | TBD at P4 execution. |
| **C-71-5** | Strand B observability cardinality: per-gate routing row vs generic? | pending P5 inline-ratify | every CI gate has its own row | TBD at P5 execution. |
...
| **D-IH-71-P** | Strand C1 tag-now-vs-hold ratification (P3) | pending P3 (renumbered from prior -O slot) |
| **D-IH-71-Q** | Strand C2 column-vs-table ratification (P4) | pending P4 |
| **D-IH-71-R** | Pack A4 ratification (render-ownership coverage thresholds) | pending P5 |
| **D-IH-71-S** | Strand B observability cardinality ratification (P5) | pending P5 |
| **D-IH-71-CLOSURE** | Initiative closure (P6) | pending P6 |
