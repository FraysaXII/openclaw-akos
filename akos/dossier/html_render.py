...
figure.dossier-screenshot {{ margin: 1.25rem 0; padding: 0.75rem;
  background: var(--c-card); border: 1px solid var(--c-border);
  border-radius: 0.5rem; }}
figure.dossier-screenshot img {{ width: 100%; height: auto;
  border-radius: 0.25rem; display: block; }}
figure.dossier-screenshot figcaption {{ margin-top: 0.5rem; padding-top: 0.5rem;
  border-top: 1px solid var(--c-border);
  color: var(--c-muted-foreground); font-size: 0.9rem; }}
figure.dossier-screenshot figcaption code {{ font-size: 0.85em; }}
...
def _render_screenshots_appendix(paths: list[Path] | None) -> str:
    """Render Appendix A as inline base64 PNGs (D-IH-48-K).

    Closes the gap between ``runner.take_browser_screenshots`` (which only
    creates the directory) and the operator-visible deliverable. Inline
    base64 keeps D-IH-48-I "standalone HTML" intact: no relative path, no
    CDN, no JS — the operator can email a single file.

    Best-effort: missing/unreadable PNGs are skipped silently; empty list
    yields an empty string (no appendix when no screenshots present).
    """
    if not paths:
        return ""
    figures: list[str] = []
    for p in paths:
        try:
            ...
            continue
        # Caption: filename stem with separators normalized
        stem = p.stem.replace("-", " ").replace("_", " ")
        caption = stem[0].upper() + stem[1:] if stem else p.name
        figures.append(
            f'    <figure class="dossier-screenshot">\n'
            f'      <img src="data:image/png;base64,{b64}" '
            f'alt="{_html_escape(p.name)}" loading="lazy">\n'
            f'      <figcaption>{_html_escape(caption)} '
            f'<code>{_html_escape(p.name)}</code></figcaption>\n'
            f'    </figure>'
        )
    if not figures:
        return ""
    ...
    return (
        ...
        f'  <summary><strong>Appendix A</strong> &mdash; Operator screenshots '
        f'({len(figures)})</summary>\n'
...
def render_dossier_html(run: Any, screenshots: list[Path] | None = None) -> str:
    ...
    When ``screenshots`` is provided, an "Appendix A — Operator screenshots"
    block is appended with each PNG inlined as base64 (D-IH-48-K). Keeps the
    standalone-file invariant intact while surfacing what was previously a
    silent on-disk artifact.
    ...
    body = "\n".join(body_parts)
    appendix = _render_screenshots_appendix(screenshots)
